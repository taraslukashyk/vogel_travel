...
async function check() {
  try {
    ...
    const { data: services, error: sErr } = await supabase.from('services').select('id, title, is_published, slug').limit(2);
    if (sErr) console.error('Services Error:', sErr);
    else console.log(JSON.stringify(services, null, 2));

    console.log('\n--- Blog Posts ---');
    const { data: blog, error: bErr } = await supabase.from('blog_posts').select('id, title, is_published, slug').limit(2);
